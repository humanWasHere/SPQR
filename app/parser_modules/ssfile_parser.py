    ...
    def ssfile_to_dataframe(self):
        ...
        if self.in_genepy:  # à check autrement -> data in ssfile or external -> True/False is genepy file
            ...
            with open(self.ssfile, 'r') as f:
                ...
                # WARNING : first line of the ssfile should be column name - WARNING 2 : words must be space separated -> tabulations makes a new column
                max_column_number = max(len(line.split('\t')) for line in f)  # number of max column detected with longest line of the file (by iteration)
            ...
            if header_column_number != max_column_number:  # random check
                ...
                print("Le nombre de colonne dans le header est {} et le nombre de colonne max est {}.".format(header_column_number, max_column_number))  # if this line displays the same number it's a win !
            print(df.head(500).to_string())
...
# est ce qu'on fill les NaN values avec des 0 ? -> génant pour RCPD ? Post treatment for us in recipe creation ? -> for me -> if NaN value: pass
